...
predict_by_yields = function(data)  {
    ...
    fit_rf = caret::train(x = df_predicting %>% 
                                  na.omit %>% 
                                  dplyr::select(-date,-Value) %>% 
                                  data.frame(),
                             y = df_predicting %>% 
    ...
    df_predicted =
      df_predicting %>% 
      dplyr::mutate(pred_value=predict(fit_rf,.)) %>% 
      dplyr::select(date,pred_value,Value) %>%
...
```{r fig.width = 15, fig.height = 15 }
draw_charts(data = df_STOCKS_ENERGY)
```

## ETF: DISCRETIONARIES

```{r fig.width = 15, fig.height = 15 }
draw_charts(data = df_STOCKS_DISCRETIONARIES)
```

## ETF: FINANCIALS

```{r fig.width = 15, fig.height = 15 }
draw_charts(data = df_STOCKS_FINANCIALS)
```

## ETF: COMMUNICATION

```{r fig.width = 15, fig.height = 15 }
draw_charts(data = df_STOCKS_COMMUNICATION)
```

## ETF: HEALTHCARE

```{r fig.width = 15, fig.height = 15 }
draw_charts(data = df_STOCKS_HEALTHCARE)
```

## ETF: SMALLCAP

```{r fig.width = 15, fig.height = 15 }
draw_charts(data = df_STOCKS_SMALLCAP)
```

## ETF: TECHNOLOGY

```{r fig.width = 15, fig.height = 15 }
draw_charts(data = df_STOCKS_TECHNOLOGY)
```
