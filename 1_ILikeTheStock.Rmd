...
fit_draw_fibonacci = function(low,high) {
  ...
  list(geom_hline(yintercept = fibonaccis,linetype=2,color="gray60" ),
       annotate(geom="text",x=Sys.Date(),y=fibonaccis,label=fibonaccis))
}
```
...
getSymbols("^GSPC", env = NULL) %>% 
  ...
  geom_ma(ma_fun = SMA, n =5*365.25,color="red",size=1) +
  geom_ma(ma_fun = SMA, n =4*365.25,color="red",size=1) +
  geom_ma(ma_fun = SMA, n =3*365.25,color="red",size=1) +
  geom_ma(ma_fun = SMA, n =2*365.25,color="red",size=1) +
  geom_ma(ma_fun = SMA, n =1*365.25,color="green",size=1) +
  geom_ma(ma_fun = SMA, n =0.5*365.25,color="blue",size=1) +
...
getSymbols("^OMXC25", env = NULL) %>% 
  as.data.frame(.) %>% 
  dplyr::mutate(Date=ymd(row.names(.))) %>% 
  ggplot(.,aes(x=Date,y=OMXC25.Close)) +
  geom_line() +
  scale_x_date(date_breaks = "1 year",date_labels = "%Y") +
...
getSymbols("BTC-USD", env = NULL) %>% 
  as.data.frame(.) %>% 
  dplyr::mutate(Date=ymd(row.names(.))) %>% 
  # filter(Date>=ymd("2020-01-01")) %>% 
  ggplot(.,aes(x=Date,y=`BTC-USD.Close`)) +
  geom_line() +
...
tickers_filtered = df_quotes %>% 
  group_by(Sector) %>%
  top_n(n=20,wt=`Market Capitalization`) %>% 
  ungroup()
