...
rm(list=ls())
  ...
  files = list.files(path = "~/Dropbox/R/Stocks/Data",full.names = T,pattern = ".csv")

  dataset = rbindlist(lapply(files,function(csv) read.csv(csv))) %>%
    as_tibble() %>%
    dplyr::mutate(Market.Cap=Market.Cap/1000000000) %>%
    dplyr::filter(Market.Cap>10) %>%
    arrange(desc(Market.Cap)) %>%
    dplyr::select(Symbol,Name,Sector,Industry) %>% 
    distinct()

  symbols = unique(dataset$Symbol)
  ...
  # Stock data
  stock_samples = 200
  sampled_stocks = dataset %>% sample_n( stock_samples ) %>% pull(Symbol)
  ...
  for (symbol in sampled_stocks) {
    ...
    stock_data = bind_rows( stock_data , stock_symbol)
  }
  stock_data
```
